...
function useLeadLookup(query: string) {
  return useQuery({
    queryKey: ['lead-lookup', query],
    queryFn: async () => (await api.get(`/erp/lead-lookup?q=${encodeURIComponent(query)}`)).data,
    ...
    <div className="border border-slate-200 rounded-xl p-4 space-y-2 bg-white">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <span className="flex items-center gap-1 text-sm text-slate-600 mt-1">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-600">
        {lead.destination && (
          <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-slate-400" />{lead.destination}</span>
        )}
        {showSales && lead.assignedTo && (
          <span className="text-xs text-slate-500">Assigned: <span className="font-medium">{lead.assignedTo.name}</span></span>
        )}
        {lead.campaign && <span className="text-xs text-slate-500">{lead.campaign.name}</span>}
      </div>
      {lead.email && <p className="text-xs text-slate-500">{lead.email}</p>}
      {lead.message && (
        <p className="text-xs text-slate-500 bg-slate-50 rounded px-2 py-1.5 line-clamp-3">{lead.message}</p>
      )}
      <p className="text-[10px] text-slate-400">Not booked yet — still in the sales pipeline</p>
    </div>
  );
}
    ...
    <div className="border border-slate-200 rounded-xl p-4 space-y-3 bg-white">
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-base font-semibold text-slate-900 mt-1">{lead.name}</p>
        </div>
...
export default function LeadLookup() {
  ...
  const { data, isFetching } = useLeadLookup(query);
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        title="Search by name, phone, or booking ID"
      >
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
              ...
              <input
                ...
                placeholder="Name, phone, or Booking ID (BKG-…)"
            ...
            <div className="max-h-[70vh] overflow-y-auto p-3 space-y-3">
              {query.trim().length < 3 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-slate-400 mt-1">Search by lead name, phone number, or booking ID</p>
                </div>
              ) : isFetching ? (
                <p className="text-center text-sm text-slate-400 py-8">Searching…</p>
              ) : results.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-sm text-slate-400">No leads found for "{query}"</p>
